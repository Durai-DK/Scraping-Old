...
from Scraping.Accessories.Py.Form.Product_Details import product, competitor, path
    ...
    def amazon(self):
        if self.ws.cell(row=self.row_num, column=4).value != "N/A":
            ...
            try:
                ...
                driver.implicitly_wait(3)
                print("Product Name :", driver.find_element(By.ID, "productTitle").text)
                new_ws.cell(row=self.row_num, column=6).value = driver.find_element(By.ID, "productTitle").text
                try:
                    for box_price in driver.find_elements(By.ID, "apex_desktop"):
                        for price in box_price.find_elements(By.CLASS_NAME, "a-price"):
                            print("Process :Amazon p-1")
                            print("Product Price :", price.find_element(By.CLASS_NAME, "a-price-whole").text)
                            new_ws.cell(row=self.row_num, column=7).value = \
                                price.find_element(By.CLASS_NAME, "a-price-whole").text
                except:
                    print("Process :Amazon p-2")
                    print("Product Price :", driver.find_element(By.CLASS_NAME, "apexPriceToPay").text)
                    new_ws.cell(row=self.row_num, column=7).value = \
                        driver.find_element(By.CLASS_NAME, "apexPriceToPay").text[1:]
            ...
            try:
                ...
                try:
                    if driver.find_element(By.ID, "ContentPlaceHolder1_fillprice").text != None:
                        ...
                        try:
                            print("Process : p-v-1")
                            print("Product Price :", driver.find_element(By.XPATH, '//*[@id="ContentPlaceHolder1_fillprice"]/div[1]/div[1]/span[2]/span').text)
                            ...
                            print("Process : p-v-2")
                            print("Product Price :", driver.find_element(By.XPATH, '//*[@id="ContentPlaceHolder1_fillprice"]/div/span[2]/span').text)
    ...
    def reliance(self):
        if self.ws.cell(row=self.row_num, column=7).value != "N/A":
            ...
            try:
                ...
                new_ws.cell(row=self.row_num, column=16).value = price[1:]

                print("Product Price :", price[1:])
            except:
                pass
...
class RunCompression:
    ...
    def save(self, **kwargs):
        if kwargs.get("head") == "Accessories":
            ...
            new_wb.save(path["Mobile"]["Save"]+date + ".xlsx")
            ...
            new_wb.save(path["KA"]["Save"]+date + ".xlsx")