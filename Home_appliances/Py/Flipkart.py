...
save_ws.cell(row=1, column=5).value = "Flipkart Price"
save_ws.cell(row=1, column=6).value = "Flipkart Url"
...
# for r in range(2, 101):
    ...
    if ac_ws.cell(row=r, column=10).value != "NA":
        save_ws.cell(row=r, column=6).value = ac_ws.cell(row=r, column=10).value
        print("#" * 150)
        print("Flipkart")
        print('Range: ', r)
        print("Flipkart Url :",ac_ws.cell(row=r, column=10).value)

        try:
            driver.get(url=ac_ws.cell(row=r, column=10).value)
            time.sleep(2)

            print("Flipkart Price = ", driver.find_element(By.CLASS_NAME, "_30jeq3").text[1:])
            save_ws.cell(row=r, column=5).value = driver.find_element(By.CLASS_NAME, "_30jeq3").text[1:]
        except:
            pass

    save_wb.save(r"D:\Durai\Scraping\Home_appliances\Save Date's\Save Files\Total Scraping\Home Application Flipkart "+date+".xlsx")
